...
lambda = 1.0*count #lambda needs to be a float so enter 1.0 instead of 1
maxIter = 100000
betaOld = zeros(p)+1
Xbeta = X*betaOld
probOld = expit(Xbeta)
...
errorcplot = plot(x=x,y=epsvec[5:iter],Geom.line,Guide.xlabel("Iterations"),Guide.ylabel("Errors"),Guide.title("Errors"))
draw(PDF(join(["errorcplot",string(count),"-",string(notsparse),".pdf"]), 16cm, 12cm), errorcplot)
trueerrorplot = plot(x=x,y=trueerrorvec[5:iter],Geom.line,Guide.xlabel("Iterations"),Guide.ylabel("True Beta minus Optimum Beta"),Guide.title("True Beta minus Optimum Beta"))
