...
#line 415 "genlasso.nw"
    if(verb)
      printf("  -->\tStepping onto the border of the L1 ball.\n");
    rho_up  = rho = 1.0;
    rho_low = 0.0;
...
#line 449 "genlasso.nw"
  for(j=0; j<num_nz_x; j++)
    beta[nz_x[j]] = btmp[j]+rho*step[j];
  if(verb)
    printf("  -->\tRemoving variable: %d",nz_x[to_del]+1);
...
#line 461 "genlasso.nw"
  if(verb)
    if(b_1norm < (1-prec)*t)
      printf(", and stepping into the interior of the L1 ball\n");
    else
      printf("\n");
...
if(verb){
  printf("\n\n++++++++++++++++++++++++++++++\n");
  printf("Solving problem number %ld with bound %f\n", i+1, pt[i]);
  printf("++++++++++++++++++++++++++++++\n");
}
