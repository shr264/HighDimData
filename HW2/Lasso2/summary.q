                         ...
                         sigma = NULL)
{
  ...
  if(correlation) {
    ...
    correl <- correl/se[rep(1:p,rep(p,p))]
  }else{
    correl <- NULL
  }
...
summary.gl1ce <- function(object,
                          dispersion = NULL,
                          correlation=F)
{
  if(correlation) {
    stop(paste("The \"correlation\" argument is not currently implemented",
               " for gl1ce objects", sep=""))
  }
  coef <- coef(object)
  cnames <- labels(object)
  resid <- residuals(object, type="deviance")
  fv <- fitted(object)
  family <- object$family
  iter <- object$iter
  coef <- array(coef, length(coef))
  dimnames(coef) <- list(c("(Intercept)",cnames))
