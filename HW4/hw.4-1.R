...
grouplasso = function(lambda1,p,n,y,X,truebeta,oldbeta,beta,blocksize,maxiter,tol,group.indx,weights,group){

if(missing(maxiter)){maxiter = 100}
if(missing(weights)){weights = 1/sqrt(blocksize)}
...
#code for sparse group lasso
sparsegrouplasso = function(lambda1,lambda2,p,n,y,X,truebeta,oldbeta,beta,blocksize,maxiter,tol,group.indx,weights,group){
...
for (i in 1:numTests){
    ...
    (TrainingResults = sparsegrouplasso(lambda1,lambda2,p,nTrain,yTrain,XTrain,truebeta,oldbeta,beta,blocksize,maxiter,tol,group.indx,weights,group))
...
#turns out lambda_1 = 210 was ideal
temp = sparsegrouplasso(210,0,p,n,y,X,truebeta,oldbeta,beta,blocksize,100,10^(-5),group.indx,weights,group)
y = temp$beta
x = temp$truebeta
...
temp = sparsegrouplasso(210,0,p,n,y,X,truebeta,oldbeta,beta,blocksize,100,10^(-6),group.indx,weights,group)
y = temp$beta
x = temp$truebeta
...
plot(40*(seq(1:20)-1)+100,avgerror,type='l',col='blue',ylab='MSE',xlab=expression(lambda[2]),main = 'MSE for Group Lasso')
dev.off()

temp = sparsegrouplasso(210,250,p,n,y,X,truebeta,oldbeta,beta,blocksize,100,10^(-6),group.indx,weights,group)
y = temp$beta
x = temp$truebeta
...
temp = sparsegrouplasso(210,120,p,n,y,X,truebeta,oldbeta,beta,blocksize,100,10^(-6),group.indx,weights,group)
y = temp$beta
x = temp$truebeta
